...
class FeatureEngineer:
    ...
    def __init__(self, model_id, grain=None, stage=None) -> object:
        """
        Initialises FeatureEngineer object.
        :param model_id:
        :param grain:
        :param stage:
        """
        ...
        self._stage = stage
        # Columns in model:
        self._coltyp_tgt = []  # list of target columns - more than one in case of ensemble or multi-class binary setup
        self._coltyp_double = []  # column names for all numerics
        self._coltyp_string = []  # column names for all categorical
        self._train_test_split = None
        self._processed_data = None

        # Set attribute data on load. TODO: This is misleading as it will only load on object create. Should probably
        #  remove. Should load data on demand only, much safer.
        if self._model_id == "long_live":
            self.load_train_test_split()
            self.load_processed_battery_data()

    def get_cycle_example(self, cell_value, summary_idx, cycle_idx, scaling_factors):
        """
    ...
    def _write_to_tfrecords(self, train_test_split=None):
        """
        ...
        if not os.path.exists(cst.DATASETS_DIR):
            os.mkdir(cst.DATASETS_DIR)

        scaling_factors = self.calculate_and_save_scaling_factors(self._processed_data, train_test_split,
                                                                  cst.SCALING_FACTORS_DIR)

        if train_test_split is None:
            # Write all cells into one directory
            for cell_name, cell_data in self._processed_data.items():
                self.write_single_cell(cell_name, cell_data, cst.DATASETS_DIR, scaling_factors)
        else:
            ...
            for split_name, split_indexes in train_test_split.items():
                split_data_dir = os.path.join(cst.DATASETS_DIR2, split_name)
                # create directories
                if not os.path.exists(split_data_dir):
                    os.mkdir(split_data_dir)
                split_batteries = {idx: self._processed_data[idx] for idx in split_indexes}
                for cell_name, cell_data in split_batteries.items():
                    self.write_single_cell(cell_name, cell_data, split_data_dir, scaling_factors)

    def write_single_cell(self, cell_name, cell_data, data_dir, scaling_factors):
        """
        ...
        with tf.io.TFRecordWriter(str(filename)) as f:
            for summary_idx, cycle_idx in enumerate(cell_data["cycles"].keys()):
                cycle_to_write = self.get_cycle_example(cell_data, summary_idx, cycle_idx, scaling_factors)
    ...
    def get_create_cell_dataset_from_tfrecords(self, window_size, shift, stride, drop_remainder):
        def create_cell_dataset_from_tfrecords(file):
            ...
            dataset = dataset.flat_map(self.get_flatten_windows(window_size))
            return dataset

        return create_cell_dataset_from_tfrecords

    def create_dataset(self, data_dir, window_size, shift, stride, batch_size,
        ...
        assembled_dataset = filepath_dataset.interleave(
            self.get_create_cell_dataset_from_tfrecords(window_size, shift, stride,
                                                        drop_remainder),
    ...
    def calculate_and_save_scaling_factors(self, data_dict, train_test_split, csv_dir):
    ...
    def load_train_test_split(self):
        ...
        """
        self._train_test_split = pickle.load(open(cst.TRAIN_TEST_SPLIT, "rb"))
        return self._train_test_split

    # Set data attribute.
    def load_processed_battery_data(self):
        self._processed_data = pickle.load(open(cst.PROCESSED_DATA, "rb"))
...
def main():
    ...
    feature_engineer._write_to_tfrecords()
